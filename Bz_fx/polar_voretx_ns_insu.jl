...
function ImplementBCs_cheb!(Op, diffMatrix, grid, params)
    ...
    kron!(Op.𝒟²ˣᶻᴺ, diffMatrix.𝒟²ˣ, diffMatrix.𝒟ᶻᴺ  )
    kron!(Op.𝒟³ˣᶻᴺ, diffMatrix.𝒟³ˣ, diffMatrix.𝒟ᶻᴺ  )

    kron!(Op.𝒟²ˣ²ᶻᶜ, diffMatrix.𝒟²ˣ, diffMatrix.𝒟²ᶻᶜ)
    kron!(Op.𝒟ˣ³ᶻᴰ,  diffMatrix.𝒟ˣ,  diffMatrix.𝒟³ᶻᴰ)

    kron!(Op.𝒟ˣ³ᶻᴺ,  diffMatrix.𝒟ˣ,  diffMatrix.𝒟³ᶻᴺ)
...
function construct_matrices(Op, mf, params)
    ...
    #* 1. uᶻ equation (bcs: w = ∂ᶻw = 0 @ z = 0, 1)
    𝓛₁[:, 1:1s₂] = 1.0 * params.E * Dᶜ⁴

    𝓛₁[:, 1s₂+1:2s₂] = -1.0 * Op.𝒟ᶻᴰ 
                    
    𝓛₁[:, 3s₂+1:4s₂] = (1.0params.Λ * mf.B₀ * D² * Op.𝒟ᶻᴺ 
                      + 1.0params.Λ * mf.∇ˣˣB₀ * Op.𝒟ᶻᴺ
                      + 2.0params.Λ * mf.∇ˣB₀ * Op.𝒟ˣᶻᴺ
                      - 2.0params.Λ * mf.∇ˣˣB₀ * H * Op.𝒟²ˣᶻᴺ
                      - 1.0params.Λ * mf.∇ˣB₀  * H * Op.𝒟³ˣᶻᴺ
                      - 1.0params.Λ * mf.∇ˣˣˣB₀ * H * Op.𝒟ˣᶻᴺ
                      + 1.0params.Λ * params.kₓ^2 * mf.∇ˣB₀ * H * Op.𝒟ˣᶻᴺ
                      + 1.0params.Λ * mf.∇ˣB₀ * H * Op.𝒟ˣ³ᶻᴺ)
    ...
    𝓛₁[:, 4s₂+1:5s₂] = (-2.0im * params.Λ * params.kₓ * mf.∇ˣˣB₀ * H * Op.𝒟ˣ
                      ...
                      + 1.0im * params.Λ * params.kₓ * mf.∇ˣB₀ * H * Op.𝒟²ᶻᴰ)

    #* 2. ωᶻ equation (bcs: ζ = 0 @ z = 0, 1)
    𝓛₂[:, 1:1s₂] = 1.0 * Op.𝒟ᶻᶜ
    𝓛₂[:, 1s₂+1:2s₂] = 1.0params.E * D²
    𝓛₂[:, 3s₂+1:4s₂] = -1.0im * params.kₓ * params.Λ * mf.∇ˣB₀ * H * Op.𝒟²ᶻᴺ
    𝓛₂[:, 4s₂+1:5s₂] = (1.0params.Λ * mf.B₀ * Op.𝒟ᶻᴰ 
                      + 1.0params.Λ * mf.∇ˣB₀ * H * Op.𝒟ˣᶻᴰ)
    ...
    #* 4. bᶻ equation (insulating wall: bcs: ∂ᶻbᶻ = 0 @ z = 0, 1)
    ...
    #* 5. jᶻ equation (insulating wall: bcs: jᶻ = 0 @ z = 0, 1)
...
@with_kw mutable struct Params{T<:Real} @deftype T
    L::T        = 4.0          # horizontal domain size (MATCHING MATLAB Asp=4)
    ...
    Nx::Int64   = 84           # no. of x-grid points (MATCHING MATLAB)
...
function solve_PolarVortex()
    ...
    Nx::Int = params.Nx
    Nz::Int = params.Nz 
    filename = "benchmark/eigenvals_ns_insu"  * "_elssaer" * string(Λ) * "_" * string(Nz) * string(Nx) * ".jld2"
